...
function App() {
  ...
  const calculation = useMemo(() => {
    ...
    const dailyTask = DAYS.map((day, index) => tasks.filter((task) => !task.deferred && task.deadline === day).reduce((sum, task) => sum + task.estimatedHours, 0) + tasks.filter((task) => !task.deferred && !task.deadline).reduce((sum, task) => sum + task.estimatedHours / 7, 0));
    const dailyMargins = DAYS.map((_, index) => 24 - sleepHours - decompHours - dailyFixed[index] - dailyTask[index]);
    const highLoad = dailyMargins.map((dailyMargin, index) => dailyMargin < 5 || dailyTask[index] > 6);
    let currentRun = 0;
    let longestRun = 0;
    highLoad.forEach((isHigh) => {
      currentRun = isHigh ? currentRun + 1 : 0;
    ...
    return { fixedTotal, recoveryBlockTotal, tier2Total, availableCapacity, flexTotal, margin, dailyMargins, dailyTask, longestRun, distributionWarning, status };
  ...
  const projectedMargin = calculation.margin - (Number.isFinite(draftHours) ? draftHours : 0);
  ...
  const sleepImpact = projectedMargin < 0 ? Math.min(sleepHours, Math.max(0.5, Math.round(Math.abs(projectedMargin) * 0.6 * 2) / 2)) : 0;
  const predictedHighLoadDays = projectedMargin < 0 ? Math.max(3, calculation.longestRun) : calculation.longestRun;
  const triageItems = tasks.filter((task) => !task.deferred);
  ...
  return <div className="dashboard-page">
    ...
    <div className="dashboard-grid">
      <section className="hero-margin-card" style={{ background: status.soft }}>
        ...
        <div className="margin-footer"><span>{status.copy}</span><span className="trend-copy"><span className="trend-arrow">↓</span> Shrinking <small>was 24 hrs Monday</small></span></div>
...
type CalculationShape = { fixedTotal: number; recoveryBlockTotal: number; tier2Total: number; availableCapacity: number; flexTotal: number; margin: number; dailyMargins: number[]; dailyTask: number[]; longestRun: number; distributionWarning: boolean; status: ReturnType<typeof statusFor> };
type ReturnType<T> = T extends (...args: any[]) => infer R ? R : never;
const useCalculationShape = null as unknown as () => CalculationShape;

function CommitmentMirror({ draftName, draftHours, draftDeadline, draftSuggestion, projectedMargin, projectedStatus, consequenceDay, sleepImpact, predictedHighLoadDays, showActions, confirmBreach, setDraftName, setDraftHours, setDraftDeadline, setShowActions, onAddAnyway, onTriage, onSplit, onFindSlot, onDefer }: { draftName: string; draftHours: number; draftDeadline: string; draftSuggestion: Suggestion; projectedMargin: number; projectedStatus: ReturnType<typeof statusFor>; consequenceDay: string; sleepImpact: number; predictedHighLoadDays: number; showActions: boolean; confirmBreach: boolean; setDraftName: (value: string) => void; setDraftHours: (value: number) => void; setDraftDeadline: (value: string) => void; setShowActions: (value: boolean) => void; onAddAnyway: () => void; onTriage: () => void; onSplit: () => void; onFindSlot: () => void; onDefer: () => void }) {
