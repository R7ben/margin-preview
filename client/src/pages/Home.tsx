...
const statusFor = (margin: number) => {
  if (margin < 0) return { label: "At risk", color: "#C1121F", soft: "#FFF5F5", copy: "Recovery floor exceeded" };
  if (margin <= 5) return { label: "At risk", color: "#C1121F", soft: "#FFF8F8", copy: "Very little margin remains" };
  if (margin <= 20) return { label: "Tight", color: "#F4A261", soft: "#FFF9F1", copy: "Margin is getting narrow" };
  return { label: "On track", color: "#2D6A4F", soft: "#F3F8F5", copy: "Room remains around your commitments" };
};

const statusForRiskLevel = (riskLevel: DayRisk["riskLevel"]) => riskLevel === "high" ? statusFor(0) : riskLevel === "medium" ? statusFor(10) : statusFor(21);
const statusForBurnoutScore = (score: number) => score >= 75 ? statusFor(0) : score >= 50 ? statusFor(10) : statusFor(21);
...
const flexibilityTagForTask = (task: FlexibleTask, today = new Date()) => {
  const urgency = taskUrgency(task.deadline, today);
  if (urgency.tier === "critical" || urgency.tier === "urgent") return "Due soon";
  if (urgency.tier === "flexible") return "Can defer";
  return "Flexible";
};
...
function BurnoutRiskPanel({ atRiskDays, onRebalance }: { atRiskDays: DayRisk[]; onRebalance: (day: string) => void }) {
  if (!atRiskDays.length) return <section className="burnout-panel burnout-balanced"><h3>On track</h3><p>No days are at elevated risk. Keep protecting recovery blocks.</p></section>;
  return <section className="burnout-panel"><h3>At risk</h3>{atRiskDays.map((day) => { const dayStatus = statusForRiskLevel(day.riskLevel); return <div key={day.day} className={`risk-card risk-${day.riskLevel}`}><div className="risk-day"><strong>{fullDayName(day.day)}</strong><span style={{ color: dayStatus.color }}>{dayStatus.label} · {formatShortHours(day.totalHours)} scheduled</span></div><div className="risk-narrative">{day.riskNarrative}</div><button className="btn-secondary-sm" onClick={() => onRebalance(day.day)}>Rebalance</button></div>; })}</section>;
}
...
function Dashboard({ calculation, tasks, fixedCommitments, recoveryBlocks, showEnergyCheckIn, showMorningCheckIn, onMorningCheckInRespond, onAddQuickTask, onQuickCheck, onOpenActionSheet, recoveryQualityBlock, recoveryQualityDismissed, onRecoveryQuality, onPlanner, onReflection, weeklyPlan, showSurvivalPlan, onMoveWeeklyTask, onLockWeeklyBlock, onDismissSurvivalPlan, onShowWeeklyPlan, onTriage, onRebalance, onRecordOutcome, onEnergyRespond, lastToastTaskId, showDeprioritizedBanner, onDismissDeprioritizedBanner }: { calculation: ReturnType<typeof useCalculationShape>; tasks: FlexibleTask[]; fixedCommitments: FixedCommitment[]; recoveryBlocks: RecoveryBlock[]; showEnergyCheckIn: boolean; showMorningCheckIn: boolean; onMorningCheckInRespond: (value: MoodValue) => void; onAddQuickTask: (values: QuickAddValues) => void; onQuickCheck: () => void; onOpenActionSheet: (target: ActionSheetTarget) => void; recoveryQualityBlock: RecoveryBlock | null; recoveryQuality: "Fully" | "Partially" | "Not really" | null; recoveryQualityDismissed: boolean; onRecoveryQuality: (quality: "Fully" | "Partially" | "Not really") => void; onPlanner: () => void; onReflection: () => void; weeklyPlan: WeeklyPlan; showSurvivalPlan: boolean; onMoveWeeklyTask: () => void; onLockWeeklyBlock: () => void; onDismissSurvivalPlan: () => void; onShowWeeklyPlan: () => void; onTriage: () => void; onRebalance: (day: string) => void; onRecordOutcome: (task: FlexibleTask, outcome: TaskOutcomeValue) => void; onEnergyRespond: (response: EnergyResponse) => void; lastToastTaskId: number | null; showDeprioritizedBanner: boolean; onDismissDeprioritizedBanner: () => void }) {
  ...
  const headline = status.label === "At risk" ? "Two things need to move." : calculation.distributionWarning || anyRiskDay ? `${lowestDay} is tight.` : "You're protected this week.";
  ...
  return <div className="dashboard-page">
    ...
    <section className="card hero-margin-card">
      ...
      <div className="progress-ring-wrap">
        ...
        <div className="progress-ring-center">
          ...
          <span className="trend-copy"><span className="trend-arrow">{calculation.flexTotal > 0 ? "↓" : "→"}</span> {calculation.flexTotal > 0 ? "Flexible load included" : "No flexible load"}</span>
        </div>
      </div>
    ...
    <section className="card week-glance-card">
      ...
      {[{ key: "morning", label: "Morning", time: "09:00 – 12:00" }, { key: "afternoon", label: "Afternoon", time: "12:00 – 17:00" }, { key: "evening", label: "Evening", time: "17:00 – 21:00" }].map((slot) => {
        ...
        return <div className="daily-slot" key={slot.key}><div className="daily-slot-head"><strong>{slot.label}</strong><span>{slot.time}</span></div>{slotCommitments.map((commitment) => <button type="button" className="daily-blocked-bar daily-blocked-bar-interactive" aria-label={`Edit or delete ${commitment.name}`} key={commitment.id} onClick={() => onOpenActionSheet({ kind: "fixed", item: commitment })}><span className="flexibility-tag flexibility-tag-fixed">Fixed</span> Blocked · {commitment.name} · {commitment.startTime}–{commitment.endTime}</button>)}{slotTasks.length ? <div className="daily-task-chips">{slotTasks.map((task) => <button className={`daily-task-chip daily-task-chip-${task.category}`} key={task.id} onClick={() => onOpenActionSheet({ kind: "task", item: task })}><span className={`flexibility-tag flexibility-tag-${flexibilityTagForTask(task, today).toLowerCase().replace(/\s/g, "-")}`}>{flexibilityTagForTask(task, today)}</span> {task.name} · {formatShortHours(task.estimatedHours)}</button>)}</div> : <span className="daily-empty-slot">No tasks — margin available</span>}{slot.key === "evening" && dailyRecoveryBlocks.map((block) => <div className="daily-recovery-bar" key={block.id}>░░ Recovery block · {block.type} · {block.startTime}–{block.endTime}</div>)}</div>;
      })}
    </section>}
  ...
  return <div className="reflection-page">
    ...
    <section className="card reflection-metrics-card">
      <div className="reflection-metrics">
        ...
        <MetricCard icon={<ShieldCheck size={18} />} label="Recovery Quality" value={recoveryQualityLabel} subLabel={avgCheckInScore === null ? "Complete a daily check-in on the Today screen." : undefined} mark="✓" tone={recoveryQualityLabel === "At risk" ? "red" : recoveryQualityLabel === "Tight" ? "amber" : "green"} />
    ...
    <div className="sheet-backdrop" role="dialog" aria-modal="true">
      <div className="quick-sheet">
        ...
        <div className="quick-result">
          ...
          <span className="risk-badge" style={{ color: statusForBurnoutScore(risk.score).color }}><Sparkles size={12} /> Burnout risk {risk.score}/100 · {statusForBurnoutScore(risk.score).label} · trend {risk.trend}</span>
